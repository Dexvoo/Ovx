...
async function validateXPPreconditions(member, triggerChannel) {
    ...
    if (!levelUpChannel) {
        client.utils.LogData('XP Pre-Check', `Guild: ${guild.name} | Level-up channel not found.`, 'error');
        return null;
    }
    ...
    if (!hasPerms) {
        client.utils.LogData('XP Pre-Check', `Guild: ${guild.name} | Missing permissions in level-up channel.`, 'error');
        return null;
    }
