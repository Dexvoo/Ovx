...
async function Embed(target, color, title, description, { fields = [], ephemeral = true } = {}) {
  ...
  if (!color || (!title && title !== '') || (!description && description !== '')) {
    console.error(
      'Embed Util Error: Missing one of required parameters: color, title, or description.'
    );
    return null;
  }
  ...
  try {
    if (target instanceof BaseInteraction) {
      ...
      console.error(
        `Embed Util Error: Unsupported target type provided: ${target.constructor.name}`
      );
      return null;
    }
  } catch (error) {
    if (
      ...
      console.error(
        `Embed Util Error: Failed to send DM to user ${target.id}. They may have DMs disabled or have the bot blocked.`
      );
    } else {
...
const SendEmbed = async (
  ...
  const textComponentTitle = new TextDisplayBuilder().setContent(`# ${title}`);
  containerComponent.addTextDisplayComponents(textComponentTitle);

  const separatorComponent = new SeparatorBuilder().setDivider(true);
  containerComponent.addSeparatorComponents(separatorComponent);

  const textComponentDescription = new TextDisplayBuilder().setContent(
  ...
  if (interaction instanceof ChatInputCommandInteraction) {
    ...
    interaction.send({
      flags: [MessageFlags.IsComponentsV2, ephemeral ? MessageFlags.Ephemeral : null],
      components: [containerComponent],
      allowedMentions: { parse: [] },
    });
  }
...
async function EmbedDev(type, client, embed) {
  ...
  try {
    await client.shard.broadcastEval(
      async (shardClient, { embed, channelId, guildId }) => {
