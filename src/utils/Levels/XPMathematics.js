...
function MessageXP() {
    min = Math.ceil(XP_MINIMUM_TEXT_CHAT);
    max = Math.floor(XP_MAXIMUM_TEXT_CHAT);
...
function VoiceXP(minutes) {
    let min = Math.ceil(XP_MINIMUM_VOICE_CHAT);
    let max = Math.floor(XP_MAXIMUM_VOICE_CHAT);
    return (Math.floor(Math.random() * (max - min + 1) + min) * minutes) / COOLDOWN_VOICE_CHAT;
}
