const { SlashCommandBuilder, Colors, InteractionContextType, ApplicationIntegrationType } = require('discord.js');
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand => subcommand
            ...
            .addUserOption(option => option.setName('target').setDescription('The user to get info about (defaults to you).').setRequired(false))
        )
        .addSubcommand(subcommand => subcommand
            .setName('channel')
            .setDescription('Get information about a channel.')
            .addChannelOption(option => option.setName('target').setDescription('The channel to get info about (defaults to current).').setRequired(false))
        )
        .addSubcommand(subcommand => subcommand
            .setName('role')
    ...
    async execute(interaction) {
        ...
        if (!handler) {
            return interaction.client.utils.Embed(interaction, Colors.Red, 'Error', `Subcommand handler for '${subcommand}' not found.`);
        }
