  ...
  async execute(interaction) {
    ...
    if (!client.utils.DevCheck(user.id))
      return client.utils.Embed(
        ...
        Colors.Red,
        'Command Failed',
        `User Missing Permission: \`Developer\``
      );
...
const SendEmbedv2 = async (messageTarget, title, description, footer, thumbnail) => {
  ...
  const textComponentFooter = new TextDisplayBuilder().setContent(
  ...
  if (messageTarget instanceof CommandInteraction) {
    ...
    try {
      if (messageTarget.replied || messageTarget.deferred) {
        return await messageTarget.editReply({
          flags: [MessageFlags.IsComponentsV2],
          components: [containerComponent],
          allowedMentions: { parse: [] },
        });
      } else {
        return await messageTarget.reply({
          flags: [MessageFlags.IsComponentsV2],
          components: [containerComponent],
          allowedMentions: { parse: [] },
        });
