  ...
  async execute(interaction) {
    ...
    try {
      ...
      client.utils.Embed(
        ...
        Colors.Green,
        'Success',
        `Successfully reloaded command \`${newCommand.data.name}\`.`
      );
    } catch (error) {
      ...
      client.utils.Embed(
        ...
        Colors.Red,
        'Error',
        `There was an error while reloading command \`${commandName}\`:\n\`${error.message}\``
      );
    }
  },
};
...
function findCommandPath(category, commandName) {
  ...
  try {
    ...
    const commandFile = files.find(
      (file) => file.toLowerCase().startsWith(commandName.toLowerCase()) && file.endsWith('.js')
    );
    if (commandFile) {
