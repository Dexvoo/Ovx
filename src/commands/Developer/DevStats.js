...
async function guildLeaderboard(interaction) {
  ...
  console.log(
    `DevStats: Fetching leaderboard for guild ${guildId} with type ${type} by user ${user.tag}`
  );
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      .setColor(Colors.Red)
      .setDescription('Could not find or fetch the specified guild.');
  ...
  if (!LevelConfigData || !LevelConfigData.enabled) {
    const embed = new EmbedBuilder()
      ...
      .setDescription(`Leveling is not enabled in \`${fetchedGuild.name}\`.`);
    return interaction.editReply({ embeds: [embed] });
  }
  ...
  const typeConfig = {
    ...
    messages: {
      ...
      format: (u, data) =>
        `\`${data.rank}.\` \`@${u.username.substring(0, 15).padEnd(15)}\` | M: \`${data.totalMessages.toLocaleString()}\``,
    },
    voice: {
      sortField: 'totalVoice',
      format: (u, data) =>
        `\`${data.rank}.\` \`@${u.username.substring(0, 15).padEnd(15)}\` | V: \`${(data.totalVoice / 60).toFixed(1)}h\``,
    },
  };
  ...
  if (!config) {
    const embed = new EmbedBuilder()
      .setColor(Colors.Red)
      .setDescription('Invalid leaderboard type specified.');
    return interaction.editReply({ embeds: [embed] });
  }
  ...
  if (!allLeaderboardData || allLeaderboardData.length === 0) {
    const embed = new EmbedBuilder()
      ...
      .setDescription('No one in this guild has earned XP yet.');
    return interaction.editReply({ embeds: [embed] });
  }

  const totals = allLeaderboardData.reduce(
    (acc, curr) => {
  ...
  if (leaderboardLines.length === 0) {
    const embed = new EmbedBuilder()
      .setColor(Colors.Orange)
      .setDescription('Could not display any users. They may have left the platform.');
    return interaction.editReply({ embeds: [embed] });
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: 'Total Guild Voice',
        value: `${(totals.voice / 60).toFixed(1)} hours`,
        inline: true,
      }
    )
...
async function topServers(interaction) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        guilds
          .map(
            (g, i) =>
              `**${i + 1}.** ${g.name} (\`${g.id}\`) - ${g.memberCount.toLocaleString()} members`
          )
          .join('\n')
      )
      .setColor(Colors.Blurple)
  ...
  const results = await client.shard.broadcastEval((c) =>
    c.guilds.cache.map((g) => ({ name: g.name, id: g.id, memberCount: g.memberCount }))
  );
  ...
  const totalMembersArr = await client.shard.broadcastEval((c) =>
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `Across **${totalGuilds.toLocaleString()}** servers with a total of **${totalMembers.toLocaleString()}** users.`
    )
    .setColor(Colors.Blurple)
    ...
    .addFields(
      {
        name: 'Server',
        value: sortedGuilds.map((g) => `-# ${g.name.substring(0, 25)}`).join('\n'),
        inline: true,
      },
      {
        name: 'Members',
        value: sortedGuilds.map((g) => `-# ${g.memberCount.toLocaleString()}`).join('\n'),
        inline: true,
      },
      {
        name: 'Server ID',
        value: sortedGuilds.map((g) => `-# ${g.id}`).join('\n'),
        inline: true,
      }
    )
    .setFooter({ text: `Requested by ${user.username}`, iconURL: user.displayAvatarURL() })
