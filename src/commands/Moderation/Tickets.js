...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) => subcommand.setName('lock').setDescription('Lock a ticket'))
    .addSubcommand((subcommand) => subcommand.setName('unlock').setDescription('Unlock a ticket')),
  /**
  ...
  async execute(interaction) {
    ...
    if (!handlers[`ovx-ticket-${subcommand}`])
      return interaction.client.utils.Embed(
        interaction,
        Colors.Red,
        'Tickets | Not Found',
        `The subcommand \`${subcommand}\` does not exist.`
      );
    ...
    if (!hasPermission)
      return interaction.client.utils.Embed(
        interaction,
        Colors.Red,
        'Tickets | Missing Permissions',
        `Bot Missing Permissions: \`${missingPermissions}\` in ${channel}`
      );

    const TicketData = await TicketInstance.findOne({
      channelId: channel.id,
      guildId: guild.id,
    });
    const TicketConfigData = await TicketCache.get(guild.id);
    const isAdmin =
      member.roles.cache.has(TicketConfigData?.adminRoleId) ||
      member.permissions.has(PermissionFlagsBits.Administrator);
    const isMod =
      member.roles.cache.has(TicketConfigData?.supportRoleId) ||
      member.permissions.has(PermissionFlagsBits.ManageMessages);
    const ticketOwner = TicketData
      ? await guild.members.fetch(TicketData.memberId).catch(() => null)
      : null;
