...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('moderation')
    .setDescription(
      'Ban, unban, kick, mute, unmute, purge messages, or change the nickname of a user'
    )
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addUserOption((option) =>
          option.setName('user').setDescription('The user to remove a mute').setRequired(true)
        )
    )
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName('nickname').setDescription('The new nickname').setRequired(false)
        )
    ),
...
async function BanUser(interaction) {
  ...
  if (!member.permissions.has(PermissionFlagsBits.BanMembers))
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Ban',
      `User Missing Permissions | \`BanMembers\``
    );
  if (!botMember.permissions.has(PermissionFlagsBits.BanMembers))
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Ban',
      `Bot Missing Permissions | \`BanMembers\``
    );

  // Checks
  if (targetUser.id === client.user.id)
    return client.utils.Embed(interaction, Colors.Red, 'Failed Ban', "I can't ban myself");
  if (targetUser.id === member.id)
    return client.utils.Embed(interaction, Colors.Red, 'Failed Ban', "You can't ban yourself");
  if (targetMember) {
  ...
  if (isBanned)
    return client.utils.Embed(interaction, Colors.Red, 'Failed Ban', `User already banned`);
  ...
  try {
    ...
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Ban',
      `Could not ban the user. Please check my permissions and role hierarchy.`
    );
  }
...
async function UnbanUser(interaction) {
  ...
  const isBanned = await guild.bans.fetch(targetUser.id).catch(() => null);
  if (!isBanned)
    return client.utils.Embed(interaction, Colors.Red, 'Failed Unban', `User is not banned`);
...
async function KickUser(interaction) {
  ...
  if (!targetMember)
    return client.utils.Embed(interaction, Colors.Red, 'Failed Kick', `User is not in the server`);
  ...
  if (!member.permissions.has(PermissionFlagsBits.KickMembers))
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Kick',
      `User Missing Permissions | \`KickMembers\``
    );
  if (!botMember.permissions.has(PermissionFlagsBits.KickMembers))
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Kick',
      `Bot Missing Permissions | \`KickMembers\``
    );

  // Checks
  if (targetUser.id === client.user.id)
    return client.utils.Embed(interaction, Colors.Red, 'Failed Kick', "I can't kick myself");
  if (targetUser.id === member.id)
    return client.utils.Embed(interaction, Colors.Red, 'Failed Kick', "You can't kick yourself");
  ...
  if (!LogsData)
    return client.utils.LogData('Punishment Kick', `Guild: ${guild.name} | Disabled`, 'warning');
  ...
  const description = [`User: ${targetUser}`, `Reason: ${reason || 'No reason provided'}`];

  const LogEmbed = new EmbedBuilder()
    .setColor(Colors.Red)
    .setAuthor({
      name: targetUser.username,
      iconURL: targetUser.displayAvatarURL({ size: 512, extension: 'png' }),
    })
  ...
  logChannel
    ...
    .then(() =>
      client.utils.LogData(
        'Punishment Kick',
        `Guild: ${guild.name} | ${targetUser.bot ? '🤖 Bot' : '👤 User'} @${targetUser.username}`,
        'info'
      )
    )
    .catch((err) =>
...
async function MuteUser(interaction) {
  ...
  if (!targetMember)
    return client.utils.Embed(interaction, Colors.Red, 'Failed Mute', `User is not in the server`);
  ...
  if (!member.permissions.has(PermissionFlagsBits.ModerateMembers))
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Mute',
      `User Missing Permissions | \`ModerateMembers\``
    );
  if (!botMember.permissions.has(PermissionFlagsBits.ModerateMembers))
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Mute',
      `Bot Missing Permissions | \`ModerateMembers\``
    );

  // Checks
  if (targetUser.id === client.user.id)
    return client.utils.Embed(interaction, Colors.Red, 'Failed Mute', "I can't mute myself");
  if (targetUser.id === member.id)
    return client.utils.Embed(interaction, Colors.Red, 'Failed Mute', "You can't mute yourself");
  if (targetMember.isCommunicationDisabled())
    return client.utils.Embed(
      interaction,
      Colors.Red,
      'Failed Mute',
      `${targetMember} is already muted, It ends ${client.utils.Timestamp(targetMember.communicationDisabledUntil)}`
    );
  ...
  if (!duration)
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Mute',
      `Please provide a valid duration of time e.g. 10m, 1h, 1d`
    );
  if (duration > 28 * 24 * 60 * 60 * 1000)
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Mute',
      `You can't mute a user for longer than 28 days`
    );
  ...
  try {
    await targetMember.disableCommunicationUntil(
      Date.now() + duration,
      `Muted by @${member.user.username} for: ${reason}`
    );
  } catch (error) {
    return client.utils.Embed(
      interaction,
...
async function UnmuteUser(interaction) {
  ...
  // Checks
  if (targetUser.id === client.user.id)
    return client.utils.Embed(interaction, Colors.Red, 'Failed Unmute', "I can't unmute myself");
  if (targetUser.id === member.id)
    return client.utils.Embed(interaction, Colors.Red, 'Failed Unmute', "You can't mute yourself");
  if (!targetMember.isCommunicationDisabled())
    return client.utils.Embed(
      interaction,
      Colors.Red,
      'Failed Unmute',
      `${targetMember} is not muted`
    );
  if (!targetMember.moderatable)
    return client.utils.Embed(
      interaction,
  ...
  if (member.roles.highest.position <= targetMember.roles.highest.position)
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Unmute',
      `You can\'t unmute a member with a higher role than you`
    );
...
async function PurgeMessages(interaction) {
  ...
  if (!botMember.permissions.has(PermissionFlagsBits.ManageMessages))
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Purge',
      `Bot Missing Permissions | \`ManageMessages\``
    );
...
async function NicknameUser(interaction) {
  ...
  if (targetMember.id === member.id)
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Nickname',
      "You can't change your own nickname"
    );
  if (!targetMember.moderatable)
    return client.utils.Embed(
      interaction,
  ...
  if (member.roles.highest.position <= targetMember.roles.highest.position)
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Nickname',
      `You can\'t change the nickname of a member with a higher role than you`
    );
  ...
  if (newNickname === oldNickname)
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Nickname',
      `New nickname is the same as the old nickname`
    );

  await targetMember
    .setNickname(newNickname, `Changed by @${interaction.user.username}`)
    .catch(console.error);
