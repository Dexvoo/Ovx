...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName('name')
            .setDescription('The name for the new sticker (2-32 characters).')
            .setRequired(true)
        )
        .addStringOption((option) =>
          option
            .setName('message_link')
            .setDescription('The link to the message containing the sticker.')
            .setRequired(true)
        )
        .addStringOption((option) =>
          option
            .setName('description')
            .setDescription('Optional description for the sticker.')
            .setRequired(false)
        )
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName('sticker')
            .setDescription('The sticker you want to edit.')
            .setAutocomplete(true)
            .setRequired(true)
        )
        .addStringOption((option) =>
          option.setName('name').setDescription('The new name for the sticker.').setRequired(false)
        )
        .addStringOption((option) =>
          option
            .setName('description')
            .setDescription('The new description for the sticker.')
            .setRequired(false)
        )
    )
    .addSubcommand((subcommand) =>
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName('list').setDescription('List all stickers in this server.')
    ),
  ...
  async autocomplete(interaction) {
    ...
    const filtered = stickers
      .filter((sticker) => sticker.name.toLowerCase().includes(focusedValue))
      .map((sticker) => ({
        name: sticker.name,
        value: sticker.id,
      }))
