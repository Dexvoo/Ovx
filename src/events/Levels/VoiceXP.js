...
module.exports = {
    ...
    async execute(oldState, newState) {
        ...
        if (newState.channel) {
            ...
            client.utils.LogData('Voice XP', `User @${member.user.username} joined VC #${newState.channel.name}. Tracking started.`, 'debug');
        }
        ...
        if (oldState.channel && !newState.channel) {
            ...
            if (timeInChannel < 1) { // Require at least 1 minute to earn XP
                return client.utils.LogData('Voice XP', `User @${member.user.username} left VC too soon (${timeInChannel.toFixed(1)} mins).`, 'debug');
            }
            ...
            client.utils.LogData('Voice XP', `User @${member.user.username} left VC. Granted XP for ${timeInChannel.toFixed(1)} mins.`, 'info');
        }
    }
};