...
module.exports = {
  ...
  async execute(client) {
    client.application.commands.set(client.commands.map((v) => v.data)).then(async (commands) => {
      ...
      commands.toJSON().forEach((command) => {
        ...
        subGroupCommands = command.options.filter(
          (v) => v.type === ApplicationCommandOptionType.SubcommandGroup
        );
        subCommands = command.options.filter(
          (v) => v.type === ApplicationCommandOptionType.Subcommand
        );
        ...
        if (subGroupCommands) {
          subGroupCommands.forEach((group) => {
            group.options.forEach((sub) => {
              subGroupCommandsArray.push(
                `</${command.name} ${group.name} ${sub.name}:${command.id}>`
              );
