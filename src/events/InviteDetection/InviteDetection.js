...
const INVITE_REGEX =
  /(?:https?:\/\/)?(?:www\.)?(?:discord\.(?:gg|io|me|li)|discordapp\.com\/invite)[\/\\]([a-zA-Z0-9-]{2,})/i;
  ...
  async execute(message) {
    ...
    if (!botPermissions || !botPermissions.has(PermissionFlagsBits.ManageMessages))
      return client.utils.LogData(
        'Invite Detection',
        `Missing 'Manage Messages' permission in #${channel.name}`,
        'error'
      );
    ...
    try {
      ...
      client.utils.LogData(
        'Invite Detection',
        `Caught error fetching invite '${inviteCode}'. Assuming it's an ad. Error: ${error.message}`,
        'info'
      );
    }
    ...
    try {
      ...
      const successEmbed = new EmbedBuilder()
        ...
        .setColor(Colors.Red)
        .setDescription(
          `Advertising is not allowed. Your message containing an invite link was removed.`
        )
      ...
      client.utils.LogData(
        'Invite Detection',
        `Failed to delete an invite message. Error: ${error.message}`,
        'error'
      );
    }
  },
