...
module.exports = {
	...
	async execute(client) {	
		const shardId = await client.shard.broadcastEval(
		    (c, { guildId }) => c.guilds.cache.has(guildId) ? c.guilds.cache.get(guildId).shardId : null,
		...
		if(shardId !== null) {
			...
			console.error('Developer Shard ID not found. Please ensure the bot is in the developer guild.');
