...
module.exports = {
  ...
  async execute(oldChannel, newChannel) {
    ...
    if (!LogsData?.channel?.enabled || !LogsData.channel.channelId)
      return client.utils.LogData('Channel Updated', `Guild: ${guild.name} | Disabled`, 'warning');
    ...
    if (oldChannel.type !== newChannel.type) {
      description.push(
        `**Type:** \`${ChannelType[oldChannel.type]}\` → \`${ChannelType[newChannel.type]}\``
      );
    }
    ...
    if ('userLimit' in oldChannel && oldChannel.userLimit !== newChannel.userLimit) {
      description.push(
        `**User Limit:** \`${oldChannel.userLimit || 0}\` → \`${newChannel.userLimit || 0}\``
      );
    }
    ...
    newPerms.forEach((perm, id) => {
      ...
      if (diff)
        changedPerms.push(
          `**Permission Updated for ${perm.type === 0 ? `<@&${id}>**\n${diff}` : `<@${id}>**\n${diff}`} in **#${newChannel.name}**`
        );
    });
    ...
    oldPerms.forEach((perm, id) => {
      if (!newPerms.has(id)) {
        changedPerms.push(
          `**Permission Removed for ${perm.type === 0 ? `<@&${id}>**\n${formatPermissions(perm)}` : `<@${id}>**\n${formatPermissions(perm)}`} in **#${newChannel.name}**`
        );
      }
    });
    ...
    if (positionChanged && description.length === 1)
      return client.utils.LogData(
        'Channel Updated',
        `Guild: ${guild.name} | Position change only - skipped`,
        'info'
      );
    ...
    if (description.length === 0) {
      return client.utils.LogData(
        'Channel Updated',
        `Guild: ${guild.name} | No significant changes`,
        'info'
      );
    }
    ...
    logChannel
      ...
      .then(() =>
        client.utils.LogData(
          'Channel Updated',
          `Guild: ${guild.name} | Logged changes in #${newChannel.name}`,
          'info'
        )
      )
      .catch((err) =>
...
function formatPermissions(overwrite) {
  ...
  if (allowed.length > 0)
    parts.push(`<a:OVX_Yes:1115593935746781185> **Allowed:** \`${allowed.join(', ')}\``);
  if (denied.length > 0)
    parts.push(`<a:OVX_No:1115593604073791488>  **Denied:** \`${denied.join(', ')}\``);
