...
module.exports = {
  ...
  async execute(oldMember, newMember) {
    ...
    if (!LogsData)
      return client.utils.LogData('Member Updated', `Guild: ${guild.name} | Disabled`, 'warning');
    ...
    if (oldMember.user.globalName !== newMember.user.globalName)
      LogEmbed.addFields({
        name: 'Display Name',
        value: `\`${oldMember.user.globalName || 'None'}\` → \`${newMember.user.globalName || 'None'}\``,
        inline: false,
      });
    ...
    if (oldMember.premiumSinceTimestamp !== newMember.premiumSinceTimestamp) {
      if (newMember.premiumSince)
        LogEmbed.addFields({
          name: 'Sever Boost',
          value: `${client.utils.Timestamp(newMember.premiumSince)}`,
          inline: false,
        });
      else
        LogEmbed.addFields({
          name: 'Sever Boost',
          value: `Stopped Boosting`,
          inline: false,
        });
    }
    ...
    if (addedRoles.length > 0 || removedRoles.length > 0) {
      ...
      if (removed)
        LogEmbed.addFields({
          name: `Removed Roles`,
          value: `${removed.substring(0, 1024)}`,
        });
    }

    if (LogEmbed.data.fields?.length === 0)
      return client.utils.LogData(
        'Member Updated',
        `Guild: ${guild.name} | ${newMember.user.bot ? '🤖 Bot' : '👤 User'} @${newMember.user.username} | No Changes (timeouts are expected)`,
        'error'
      );
