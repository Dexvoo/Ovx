...
module.exports = {
    ...
    async execute(message) {
        ...
        if(!LogsData) return client.utils.LogData('Message Deleted', `Guild: ${guild.name} | Disabled`, 'warning');
        ...
        if(!hasPermission) {
            await LogsCache.setType(guild.id, 'message', { enabled: false, channelId: null });
            return client.utils.LogData('Message Deleted', `Guild: ${guild.name} | Bot missing permissions in log channel, disabling logs`, 'error');
        }
        ...
        logChannel.send({ embeds: [LogEmbed] })
            .then(() => client.utils.LogData('Message Deleted', `Guild: ${guild.name} | Message by ${author.tag} deleted in #${channel.name}`, 'info'))
            .catch(err => client.utils.LogData('Message Deleted', `Guild: ${guild.name} | Failed to send log message: ${err.message}`, 'error'));
