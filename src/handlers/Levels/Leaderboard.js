...
module.exports = async function LevelsLeaderboard(interaction, context) {
  ...
  const userPromises = topUsersData.map(async (userData, i) => {
    try {
      const user =
        client.users.cache.get(userData.userId) || (await client.users.fetch(userData.userId));
  ...
  if (leaderboardLines.length === 0) {
    return client.utils.Embed(
      ...
      Colors.Orange,
      'Leaderboard Empty',
      'There is no data to display on the leaderboard yet.'
    );
  }
  ...
  try {
    ...
    const description =
      leaderboard.length > 4096 ? leaderboard.substring(0, 4093) + '...' : leaderboard;
    ...
    await client.utils.Embed(
      ...
      Colors.Red,
      'Leaderboard Failed',
      'An error occurred while displaying the leaderboard. This could be due to invalid emoji permissions or another issue.'
    );
  }
};
