...
module.exports = async function LevelAdminAdd(interaction, context) {
    ...
    if (levelsToAdd <= 0 && xpToAdd <= 0) {
        return client.utils.Embed(interaction, Colors.Red, 'Invalid Input', 'You must provide a positive number of levels or XP to add.');
    }
    if (targetUser.bot) {
    ...
    try {
        ...
        if (levelsToAdd > 0) {
            const xpForLevels = ExpForLevel(userXPData.level + levelsToAdd) - ExpForLevel(userXPData.level);
        ...
        if (finalLevel > LevelConfigData.maxLevel) {
            return client.utils.Embed(interaction, Colors.Red, 'Failed', `This action would push the user beyond the server's max level of ${LevelConfigData.maxLevel}.`);
        }
        ...
        return client.utils.Embed(interaction, Colors.Green, 'Success', `Added **${levelsToAdd}** levels and **${xpToAdd}** XP to ${targetUser}.\nNew Stats:\n**Level:** \`${finalLevel}\`\n**XP:** \`${finalXP}\``);

    } catch (error) {
        console.error("Error in LevelAdminAdd:", error);
        return client.utils.Embed(interaction, Colors.Red, 'Error', 'An unexpected error occurred while adding level/XP.');
    }
};