...
module.exports = async function LevelAdminRemove(interaction, context) {
  ...
  if (levelsToRemove <= 0 && xpToRemove <= 0) {
    return client.utils.Embed(
      ...
      Colors.Red,
      'Invalid Input',
      'You must provide a positive number of levels or XP to remove.'
    );
  }
  if (targetUser.bot) {
    return client.utils.Embed(
  ...
  try {
    ...
    return client.utils.Embed(
      ...
      Colors.Green,
      'Success',
      `Removed **${levelsToRemove}** levels and **${xpToRemove}** XP from ${targetUser}.\nNew Stats:\n**Level:** \`${finalLevel}\`\n**XP:** \`${finalXP}\``
    );
  } catch (error) {
    ...
    return client.utils.Embed(
      ...
      Colors.Red,
      'Error',
      'An unexpected error occurred while removing level/XP.'
    );
  }
};
