...
module.exports = async function LevelBlacklist(interaction, context) {
    ...
    if (!LevelConfigData.enabled) {
        return client.utils.Embed(interaction, Colors.Red, 'Failed', 'Levels are not enabled. Use `/level setup` first.');
    }
    if (!memberPermissions.has(PermissionsBitField.Flags.ManageGuild)) {
        return client.utils.Embed(interaction, Colors.Red, 'Failed', 'You need the `Manage Server` permission to use this command.');
    }
    ...
    if (!role && !channel) {
        ...
        return client.utils.Embed(interaction, Colors.Blurple, 'Current Level Blacklist', '', { fields });
    }
    ...
    if (role) {
        ...
        if (isAlreadyBlacklisted) {
            // Remove the role
            LevelConfigData.blacklisted.roleIds = LevelConfigData.blacklisted.roleIds.filter(id => id !== role.id);
    ...
    if (channel) {
        ...
        if (isAlreadyBlacklisted) {
            // Remove the channel
            LevelConfigData.blacklisted.channelIds = LevelConfigData.blacklisted.channelIds.filter(id => id !== channel.id);
    ...
    await client.utils.Embed(interaction, Colors.Blurple, 'Levels Blacklist Updated', responses.join('\n'));
};