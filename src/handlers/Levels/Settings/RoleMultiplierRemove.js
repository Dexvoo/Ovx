...
module.exports = async function RoleMultiplierRemove(interaction, context) {
    ...
    if (!Array.isArray(LevelConfigData.roleMultipliers) || !LevelConfigData.roleMultipliers.some(rm => rm.roleId === role.id)) {
        return client.utils.Embed(interaction, Colors.Red, 'Not Found', `The ${role} role does not have an XP multiplier configured.`);
    }
    ...
    return client.utils.Embed(interaction, Colors.Green, 'Multiplier Removed', `Successfully removed the XP multiplier from the ${role} role.`);
};