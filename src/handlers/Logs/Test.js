...
module.exports = async function LogsTest(interaction) {
  ...
  if (!currentConfig)
    return client.utils.Embed(
      ...
      Colors.Red,
      'Logs Setup',
      `No configuration found for \`${type}\` logs`
    );
  ...
  if (type === 'all') {
    ...
    for (const [logType, config] of Object.entries(currentConfig)) {
      ...
      if (!channel) {
        ...
        console.warn(
          `Log channel for ${logType} not found in guild ${guildId}. Removing from cache and database.`
        );
      ...
      if (!sentMessage) {
        ...
        return client.utils.Embed(
          ...
          Colors.Red,
          'Failed Setup',
          `Failed to send a test log in ${channel}`
        );
      }
  ...
  if (!channel) {
    ...
    return client.utils.Embed(
      ...
      Colors.Red,
      'Logs Setup',
      `Log channel for \`${type}\` not found. Please check the configuration.`
    );
  }
  ...
  return client.utils.Embed(
    ...
    Colors.Blurple,
    'Logs Test',
    `Successfully sent a test log for \`${type}\` logs in ${channel}`
  );
};
