...
const Cache_Logs = require('../../cache/Logs'); // Moved to the top for better practice
...
module.exports = async function LogsSetup(interaction) {
    ...
    if (type === 'server') {
        return client.utils.Embed(interaction, Colors.Blurple, `Failed Setup | Unavailable`, `Server logs will be coming soon!`);
    }
    
    if (!channel && enabled) {
        return client.utils.Embed(interaction, Colors.Red, 'Failed Setup', 'Please provide a channel to send the logs to');
    }
    ...
    if (enabled) {
        ...
        try {
            ...
            return client.utils.Embed(interaction, Colors.Red, 'Failed Setup', `Failed to send a test log in ${channel}. Please check my permissions again.`);
        }
    }
    ...
    try {
        ...
        return client.utils.Embed(interaction, Colors.Red, 'Error', 'An error occurred while saving the settings.');
    }
