...
module.exports = async function TicketSetup(interaction, context) {
  ...
  if (!enabled) {
    ...
    if (!GuildTicketConfig)
      return client.utils.Embed(
        interaction,
        Colors.Red,
        'Failed Setup',
        'Tickets are not setup on this server, `/ticket setup` to enable tickets'
      );
  ...
  if (!setupChannel)
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Setup',
      'Please provide a channel to send the ticket embed in'
    );
  if (!ticketCategory)
    return client.utils.Embed(
      interaction,
      Colors.Red,
      'Failed Setup',
      'Please provide a category to put the tickets in'
    );
  if (!archiveChannel)
    return client.utils.Embed(
      interaction,
      Colors.Red,
      'Failed Setup',
      'Please provide a channel to send the transcripts in'
    );
  if (!supportRole)
    return client.utils.Embed(
      interaction,
      Colors.Red,
      'Failed Setup',
      'Please provide a support role to ping when a ticket is created'
    );
  if (!adminRole)
    return client.utils.Embed(
      interaction,
      Colors.Red,
      'Failed Setup',
      'Please provide a admin role to ping when a ticket is created'
    );

  const botPermissionsInSetUp = [
    PermissionFlagsBits.ViewChannel,
    PermissionFlagsBits.SendMessages,
    PermissionFlagsBits.EmbedLinks,
    PermissionFlagsBits.ManageChannels,
  ];
  const [hasSetupPermissions, missingSetupPermissions] = client.utils.PermCheck(
    setupChannel,
    botPermissionsInSetUp,
    client
  );
  if (!hasSetupPermissions)
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Setup',
      `Bot Missing Permissions | \`${missingSetupPermissions.join(', ')}\` in ${setupChannel}`
    );

  const botPermissionsInArchive = [
    PermissionFlagsBits.ViewChannel,
    PermissionFlagsBits.SendMessages,
    PermissionFlagsBits.AttachFiles,
    PermissionFlagsBits.EmbedLinks,
  ];
  const [hasArchivePermissions, missingArchivePermissions] = client.utils.PermCheck(
    archiveChannel,
    botPermissionsInArchive,
    client
  );
  if (!hasArchivePermissions)
    return client.utils.Embed(
      ...
      Colors.Red,
      'Failed Setup',
      `Bot Missing Permissions | \`${missingArchivePermissions.join(', ')}\` in ${archiveChannel}`
    );
  ...
  client.utils.Embed(
    ...
    Colors.Blurple,
    'Ticket System Setup',
    `Ticket system has been setup successfully`,
    {
      fields: [
