...
module.exports = async function TicketUnlock(interaction, context) {
    ...
    if(!TicketData?.locked) return client.utils.Embed(interaction, Colors.Red, 'Tickets | Not Locked', 'This ticket is not locked.');
    ...
    await channel.permissionOverwrites.edit(TicketData.memberId, { ViewChannel: true, SendMessages: true, ReadMessageHistory: true }).catch(err => {
        ...
        return client.utils.Embed(interaction, Colors.Red, 'Tickets | Error', 'Failed to unlock the ticket. Please try again later.');
    });
    ...
    client.utils.Embed(interaction, Colors.Blurple, `Tickets | Unlock Ticket`, `This ticket has been unlocked by ${member}.`, { ephemeral: false });    
