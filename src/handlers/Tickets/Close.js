...
module.exports = async function TicketClose(interaction, context) {
  ...
  const archiveChannel = await guild.channels
    .fetch(TicketConfigData.archiveChannelId)
    .catch(() => null);
  ...
  if (archiveChannel) {
    ...
    await channel.send({
      content: 'Archive channel not found. The transcript will only be sent via DM.',
    });
  }
  ...
  if (ticketOwner) {
    try {
      await ticketOwner.send({
        embeds: [
          transcriptEmbed.setFooter({
            text: 'Here is a copy of your ticket transcript.',
          }),
        ],
        files: [transcript],
      });
      ...
      await channel.send({
        content: `Could not DM the ticket transcript to ${ticketOwner}. They may have DMs disabled.`,
      });
    }
  }
  ...
  const finalEmbed = new EmbedBuilder()
    ...
    .setDescription(
      `This channel will be deleted in 15 seconds. A transcript has been sent to the archive and to the ticket creator's DMs (if enabled).`
    )
    .setTimestamp();
  ...
  setTimeout(() => {
    channel
      .delete()
      .catch((err) => console.error(`Failed to delete ticket channel ${channel.id}:`, err));
  }, 15000); // 15 seconds
};
