...
async function endGiveaway(giveawayDoc, client) {
  ...
  if (winners.length > 0) {
    ...
    await channel.send({
      content: `Congratulations ${winners.map((id) => `<@${id}>`).join(', ')}! You won the **${giveawayDoc.prize}**!`,
      reply: { messageReference: message },
    });
  } else {
    endEmbed.addFields({ name: '😢 No Winners', value: 'There were no entrants.' });
    await channel.send({
      content: `The giveaway for the **${giveawayDoc.prize}** has ended with no entrants.`,
      reply: { messageReference: message },
    });
  }
    ...
    try {
      ...
      if (endedGiveaways.length > 0) {
        client.utils.LogData(
          'GiveawayManager',
          `Found ${endedGiveaways.length} ended giveaways to process.`,
          'info'
        );
      ...
      client.utils.LogData(
        'GiveawayManager',
        `Error checking for ended giveaways: ${error}`,
        'error'
      );
    }
  }, CHECK_INTERVAL);
