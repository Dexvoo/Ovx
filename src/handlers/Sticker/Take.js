...
module.exports = async function StickerTake(interaction) {
  ...
  if (guild.stickers.cache.size >= maxStickers) {
    return client.utils.Embed(
      ...
      Colors.Red,
      'Upload Failed',
      `This server has reached its sticker limit of **${maxStickers}**.`
    );
  }
  ...
  if (!match) {
    return client.utils.Embed(
      ...
      Colors.Red,
      'Invalid Link',
      'The provided message link is not valid.'
    );
  }
  ...
  if (name.length < 2 || name.length > 30) {
    return client.utils.Embed(
      ...
      Colors.Red,
      'Invalid Name',
      'Sticker names must be between 2 and 30 characters.'
    );
  }
  ...
  try {
    ...
    if (!channel || !channel.isTextBased()) {
      return client.utils.Embed(
        ...
        Colors.Red,
        'Fetch Error',
        'Could not find the channel from the link. Stickers must be '
      );
    }
    ...
    if (!message || message.stickers.size === 0) {
      return client.utils.Embed(
        ...
        Colors.Red,
        'No Sticker Found',
        'The message at the provided link does not contain a sticker.'
      );
    }
    ...
    if (stickerToTake.format === StickerFormatType.Lottie) {
      return client.utils.Embed(
        ...
        Colors.Red,
        'Unsupported Format',
        'Lottie stickers cannot be added to this server.'
      );
    }
    ...
    if (error.code === 50035)
      errorMessage =
        'The sticker file might be too large (max 512KB) or the name/description is invalid.';
    else if (error.code === 10014)
      errorMessage = 'Could not find the message. It may have been deleted.';
