...
module.exports = async function UserInfo(interaction) {
    ...
    const roles = member ? member.roles.cache.filter(r => r.id !== guild.id).map(r => r.toString()) : [];
    ...
    if (member) {
        fields.push(
            { name: 'Joined Server', value: client.utils.Timestamp(member.joinedAt, 'F'), inline: true },
            { name: 'Nickname', value: member.nickname ? `\`${member.nickname}\`` : 'None', inline: true },
            { name: 'Highest Role', value: member.roles.highest.toString(), inline: true },
            { name: `Roles (${roles.length})`, value: roles.length > 0 ? roles.slice(0, 5).join(', ') + (roles.length > 5 ? ` **+${roles.length - 5} more**` : '') : 'None', inline: false }
        );
    }
