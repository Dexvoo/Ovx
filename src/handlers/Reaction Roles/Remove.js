...
module.exports = async function LogsSetup(interaction) {
  ...
  if (!reactionRoleData)
    return client.utils.Embed(
      ...
      Colors.Red,
      'Reaction Roles | Error',
      `No reaction roles found for the message with ID \`${messageId}\`. Please make sure the message has reaction roles set up.`
    );

  if (!reactionRoleData.roles.some((r) => r.roleId === role.id))
    return client.utils.Embed(
      interaction,
      Colors.Red,
      'Reaction Roles | Error',
      `The role \`${role.name}\` is not set for this reaction role message.`
    );
  ...
  if (reactionRoleData.roles.length === 0) {
    ...
    return client.utils.Embed(
      ...
      `The reaction roles for the message with ID \`${messageId}\` have been removed and the message has been deleted.`
    );
  }
  ...
  const roleMenu = new StringSelectMenuBuilder()
    ...
    .addOptions(
      reactionRoleData.roles.map((role) => {
        return {
          label: guild.roles.cache.get(role.roleId).name,
          value: role.roleId,
          emoji: role.roleEmoji,
        };
      })
    );
  ...
  client.utils.Embed(
    ...
    Colors.Green,
    'Reaction Roles | Success',
    `The role \`${role.name}\` has been removed to the reaction roles for this message.`
  );
};
