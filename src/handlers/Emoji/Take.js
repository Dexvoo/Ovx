...
module.exports = async function EmojiTake(interaction) {
    ...
    for (const emojiStr of emojiStrings) {
        ...
        if ((parsed.animated && currentAnimated + success.filter(e => e.animated).length >= maxEmojis) ||
    ...
    const fields = [];
    if (success.length > 0) {
        fields.push({ name: '✅ Added', value: success.map(e => e.toString()).join(' '), inline: false });
    }
    if (failed.length > 0) {
        fields.push({ name: '❌ Failed', value: failed.join('\n'), inline: false });
    }

    await client.utils.Embed(interaction, Colors.Blurple, 'Emoji Take Results', '', { fields, ephemeral: false });
};