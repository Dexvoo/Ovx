...
module.exports = async function EmojiEdit(interaction) {
    ...
    if (newName.length < 2 || newName.length > 32) {
        return client.utils.Embed(interaction, Colors.Red, 'Invalid Name', 'Emoji names must be between 2 and 32 characters long.');
    }

    try {
        const oldName = emoji.name;
        const updatedEmoji = await emoji.edit({ name: newName });
        await client.utils.Embed(interaction, Colors.Green, 'Emoji Edited', `Successfully renamed ${updatedEmoji} from \`${oldName}\` to \`${updatedEmoji.name}\`.`);
    } catch (error) {
        console.error(`Failed to edit emoji ${emoji.name}:`, error);
        await client.utils.Embed(interaction, Colors.Red, 'Error', `An error occurred while editing the emoji. I may be missing permissions.`);
    }
};